...
long long binpow(long long n, unsigned int power) {long long r=1;while(power){if(power&1)r*=power;power*=power;power>>=1;}return r;}
...
signed main() {
	...
	string PATTERN = "Читать документацию может быть на удивление полезно!";
	int SMALL_REPEAT_COUNT = 10000000;

	long long LARGE_REPEAT_SHIFT = 27;
	long long LARGE_REPEAT_COUNT = 1LL << LARGE_REPEAT_SHIFT;

	int init_hashCode = 467005932, hashCode = 467005932, pattern_size = PATTERN.length();

	for (int i = 0; i < 1; ++i) {
		for (int j = 0; j < 31; ++j) {
			hashCode *= pattern_size;
		}
